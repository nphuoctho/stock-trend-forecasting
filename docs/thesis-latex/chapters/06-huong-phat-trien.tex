...
Hai hạng mục nêu trong đề cương không được thực hiện trong phạm vi thời gian của đồ án. Việc
chuyển chúng sang chương này là một quyết định về thứ tự ưu tiên, được ghi lại tường minh
thay vì để trống:

\begin{enumerate}[nosep]
  \item \textbf{Temporal Fusion Transformer.} TFT đã được cài đặt tối giản và chạy trên
        năm đoạn kiểm thử cuốn chiếu (Chương~\ref{ch:ket-qua}); kết quả thấp hơn LSTM hai
        nhánh nên không thay đổi kết luận. Hướng mở còn lại là TFT đầy đủ với biến tĩnh,
        đầu ra phân vị và diễn giải tầm quan trọng biến — chỉ nên thử sau khi đánh giá
        không rò rỉ thời điểm của bộ sinh cảm xúc hoàn tất.
  \item \textbf{Biến thể PhoBERT kết hợp CNN hoặc BiLSTM.} Xác thực chéo trên 1.306 mẫu
        đã cho mức đo lớp NEGATIVE tốt hơn. Đánh giá ngoài mẫu của nhánh dự báo đã có
        nhờ điểm kiểm đóng băng trước ngày kiểm thử đầu tiên
        (Mục~\ref{sec:kq-point-in-time}); khi so sánh biến thể kiến trúc, mỗi biến thể
        cũng cần một điểm kiểm đóng băng tương đương rồi chấm lại kho tin và chạy đối
        chứng.
...
Ưu tiên cao nhất trong số các hướng của chương này là mở rộng lượt đối chứng kích
thước tập sang nhiều hạt giống --- hiện chỉ có một hạt giống nên vai trò của nó là
thăm dò --- và tinh chỉnh lại bộ sinh cảm xúc trong từng cửa sổ chỉ bằng nhãn quá
khứ, thay vì một điểm kiểm đóng băng duy nhất.
...
nhưng mỗi mốc cắt phải tạo ra bảng đặc trưng và các đoạn kiểm thử riêng. Không được chọn
mốc cắt theo điểm kiểm thử tốt nhất mà không ghi số lần thử. Tương tự, có thể so sánh
ngưỡng phân vị với ngưỡng cố định, nhưng cách xác định ngưỡng phải tuân thủ tập
huấn luyện-only ở mọi trường hợp. Việc kiểm tra độ nhạy chỉ có ý nghĩa nếu các mô hình
trong các cấu hình vẫn dùng cùng ngày đánh giá và cùng cách tính chỉ số.
...
ít. Mỗi biến thể phải được so sánh bằng cùng các đoạn kiểm thử, ngân sách thử, loại bỏ đặc
trưng và chỉ số. Cần báo cáo số tham số, thời gian huấn luyện, độ nhạy theo hạt giống, số
ngày có tin thực sự được sử dụng và khoảng tin cậy. Nếu một biến thể chỉ tốt ở một phần
chia hoặc chỉ tốt ở nhóm có rất nhiều tin, không nên gọi đó là cải tiến ổn định trước khi
kiểm tra kích thước mẫu đủ lớn.
...
giai đoạn.
Kiểm thử quá khứ phải dùng dự đoán ngoài mẫu từ cuốn chiếu theo thời gian. Không dùng dự
đoán của các đoạn kiểm thử để điều chỉnh chiến lược rồi báo cáo lại trên chính các đoạn đó.
Nếu so sánh nhiều chiến lược, số lần thử phải được ghi lại và kết luận cần thận trọng trước
kiểm định nhiều lần. Các nghiên cứu dùng mô phỏng giao dịch cho thấy đánh giá kinh tế có
thể bổ sung cho chỉ số phân loại, nhưng kết quả vẫn phụ thuộc giả định mô phỏng và thị
trường \cite{Chen2021_210708721v1,Feng2018_181009936v2}.
...
\section{Các câu hỏi mở từ bốn phân tích bổ sung}

Bốn phân tích bổ sung trong Chương~\ref{ch:ket-qua} (các Mục~\ref{sec:kq-ic},
\ref{sec:kq-excess}, \ref{sec:kq-horizon} và~\ref{sec:kq-kinh-te}) đều dừng ở
giới hạn của chính mình, và mỗi giới hạn gợi ý một hướng nghiên cứu riêng. Các
hướng dưới đây là câu hỏi mở, chưa có số liệu trong đồ án này:

\begin{itemize}[nosep]
  \item \textbf{Cách đo cảm xúc giàu hơn một số vô hướng.} Điểm
        \texttt{sent\_pos\_minus\_neg} nén mỗi bài tin thành một hiệu hai xác
        suất; tương quan hạng gần 0 chỉ bác bỏ liên hệ đơn điệu của điểm số đó.
        Có thể đo cảm xúc theo khía cạnh, cường độ, chủ thể của tin, hoặc mức
        bất ngờ so với kỳ vọng thị trường, nhưng mỗi chiều bổ sung đòi hỏi nhãn
        và đối chứng riêng.
  \item \textbf{Tổng hợp theo sự kiện.} Trung bình theo ngày pha loãng tin quan
        trọng vào các ngày có nhiều tin nhỏ. Cách tổng hợp theo sự kiện, có
        trọng số theo độ tin cậy của bộ phân loại, là biến thể trực tiếp nhất,
        nhưng cần định nghĩa trước cách gom tin thành sự kiện để không mở thêm
        một lần thử không được ghi lại.
  \item \textbf{Quan hệ phi tuyến hoặc có điều kiện.} Phản ví dụ $y = x^2$ cho
        thấy một liên hệ phụ thuộc trạng thái có thể tồn tại dù tương quan hạng
        bằng 0. Có thể thử tương tác giữa điểm cảm xúc với độ biến động, vị trí
        trong phân phối lợi suất hoặc điều kiện thị trường, nhưng mỗi dạng mới
        phải được khóa trước khi xem kết quả.
  \item \textbf{Chẩn đoán kinh tế giao dịch được.} Phiên bản thực tế hơn phải
        vào lệnh từ phiên mở cửa kế tiếp trở đi, trừ phí và trượt giá, và đối
        chiếu phần biến động qua đêm vốn chiếm phần lớn biên lợi thế ở tần suất
        ngày. Sổ lệnh cũng cần trung hòa trạng thái ròng và một mẫu dài hơn để
        tách các mức Sharpe gần nhau.
  \item \textbf{Tách nhân quả ngược.} Hình mẫu ``cùng phiên có, phiên sau
        không'' cũng phát sinh nếu giá chạy trước và báo chí viết theo giọng của
        diễn biến đó. Một thiết kế tách giá dẫn dắt giọng điệu tin khỏi tin dẫn
        dắt giá cần nguồn tin có mốc giờ chính xác trong phiên, hoặc một nhóm
        đối chứng không mang nội dung doanh nghiệp.
\end{itemize}
...
\begin{enumerate}[nosep]
  ...
  \item Khóa đối chứng cơ sở, LSTM hai nhánh, loại bỏ đặc trưng và khoảng tin cậy theo lấy mẫu lặp trên các đoạn kiểm thử;
