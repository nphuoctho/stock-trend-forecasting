...
Sơ đồ dưới đây mô tả luồng xử lý của hệ thống. Khối đánh giá chấm mỗi đoạn kiểm thử cuốn
chiếu đúng một lần cùng bản kê của lần chạy; cấu hình không được điều chỉnh ngược theo
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[block, below=of models] (evaluation) {Cuốn chiếu theo thời gian, năm đoạn kiểm thử, loại bỏ đặc trưng và phân tích sai số};
...
\begin{description}[style=nextline, nosep]
  ...
  các tập huấn luyện, xác thực, kiểm thử theo ngày; chia theo thời gian yêu cầu cột
  \texttt{date} hoặc \texttt{published\_at} hợp lệ và báo lỗi nếu thiếu.
...
Tiền xử lý chỉ được ước lượng trên phần tập huấn luyện của từng cửa sổ. Mỗi đoạn kiểm thử
chỉ được đọc khi báo cáo chỉ số của chính đoạn đó; không có tập giữ lại cuối tách riêng. Nếu
thay đổi nguồn dữ liệu, mốc cắt, ngưỡng tạo nhãn hoặc cách chia tập, phải tạo bản kê mới và
không gộp kết quả giữa hai cấu hình.
