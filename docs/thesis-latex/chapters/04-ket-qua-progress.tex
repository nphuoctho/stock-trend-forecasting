...
PyTorch, Transformers, scikit-learn, vnstock và các thư viện dùng để tính đặc trưng
giá và tổng hợp cảm xúc. Bước tinh chỉnh PhoBERT dự kiến thực hiện trên GPU của Google
Colab hoặc Kaggle
vì máy phát triển hiện không có GPU phù hợp. Tên GPU, dung lượng bộ nhớ, thời gian chạy
và điểm kiểm sẽ được lưu trong bản kiểm kê cấu hình.

Hạt giống ngẫu nhiên sẽ được cố định cho bước chia dữ liệu, lấy mẫu, khởi tạo mô hình và
lấy mẫu lặp. Tập kiểm thử và tập giữ lại cuối cùng không được dùng để chọn siêu tham số.
...
quy trình và chất lượng nhãn cảm xúc, chỉ số phân loại của PhoBERT trên tập kiểm thử,
sau đó là kết quả dự báo xu hướng theo thang mô hình.

Chỉ số chính là F1 vĩ mô, kèm độ chính xác cân bằng và độ chính xác dương tính, độ bao phủ, F1 theo từng lớp
UP, FLAT và DOWN. So sánh sẽ đi từ đối chứng lớp phổ biến, đối chứng ngẫu nhiên và XGBoost chỉ giá đến LSTM chỉ
giá, LSTM hai nhánh có cảm xúc, rồi TFT nếu dữ liệu và thời gian cho phép. Phần đóng
góp của cảm xúc được tính bằng hiệu số giữa hai cấu hình trên cùng phần chia, kèm khoảng tin
cậy lấy mẫu lặp khi đủ số phần chia.
...
Bước tinh chỉnh PhoBERT và sinh xác suất cảm xúc, tiếp theo là bước ghép đặc trưng,
chạy đối chứng cơ sở và LSTM hai nhánh, hướng tới hoàn thành trước hạn nộp ngày 23
tháng 9 năm 2026. Các mốc này là kế hoạch, không phải kết quả đã đạt được.
