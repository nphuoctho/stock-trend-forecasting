...
chính thức. Dữ liệu đầu vào đã được chuẩn bị, pipeline đã chạy thử qua các bước chính,
nhưng mô hình chưa được train chính thức trên GPU. Phần này vì vậy chỉ ghi nhận trạng
thái hiện tại và cố định cách báo cáo cho giai đoạn tiếp theo.}
...
và checkpoint sẽ được lưu trong manifest.

Seed sẽ được cố định cho bước chia dữ liệu, lấy mẫu, khởi tạo mô hình và bootstrap. Test
và holdout không được dùng để chọn siêu tham số. Các quy tắc về cutoff, cách tạo nhãn
và train-only preprocessing đã được đặt ra trước khi chạy thực nghiệm chính.
...
chất lượng nhãn cảm xúc và độ đồng thuận, chỉ số phân loại của PhoBERT trên tập test,
...
Nếu cấu hình có sentiment không cải thiện, kết luận phù hợp là chưa có bằng chứng về
giá trị gia tăng trong protocol hiện tại.
